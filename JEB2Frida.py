...
# @Author: smartdone and changed by bobby
# @Date:   2019-07-30 21:31
...
# }}
# """
is_not_need_save2file = True
hook_template_normal = """
if(Java.available) {{
    Java.perform(function(){{
...
hook_template_special = """
if(Java.available) {{
    Java.perform(function(){{

        var {simple_class_name} = Java.use("{full_class_name}");

        {simple_class_name}[decodeURIComponent('{method_name}')].overload({types}).implementation = function({args}) {{
{log_code}
        }}
...
def generate_log_code(types, retval,method_class_name, method_name, class_name, args):
    #print("method_name: "+method_name)
    #method = urllib.unquote(method_name)
    #print("method: "+method)
    log_code = ""
    i = 0
    log_code = '            console.warn("[***] Hook {method_class_name}.{method_name}() succeed ......");\n'.format(method_class_name=method_class_name,method_name=method_name)
    for _type in types:
    ...
    if retval != "void":
    	if method_name.startswith("%"):
    		log_code += '            var retval = this[decodeURIComponent(\'{method_name}\')]({args});\n'.format(method_name=method_name,args=", ".join(args))
    	...
    	if method_name.startswith("%"):
    		log_code += '            this[decodeURIComponent(\'{method_name}\')]({args});\n'.format(method_name=method_name, args=", ".join(args))
        ...
        try:
            ...
            if "(" in current_addr:
                ...
                if method_name == "<init>": 
                    ...
                    hook_template = hook_template_normal
                else:
                    
                #if method_name != "$init":
                	method_name_quote = urllib.quote(method_name)
                	print(method_name_quote)
                	if method_name == method_name_quote:
                		hook_template = hook_template_normal
                	else:
                		hook_template = hook_template_special
                		method_name = method_name_quote


                type_code = generate_type_code(types)
                args_code = generate_args_code(args)
                log_code = generate_log_code(types, return_type, class_name, method_name, simple_class_name,
                                             args)
            ...
            print(e)
            print("Place the cursor in the function you want to generate the Frida code, then run the script 1")
